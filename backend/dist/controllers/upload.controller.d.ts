import { Request, Response } from 'express';
export declare const isValidFileType: (buffer: Buffer, type: "image" | "video" | "audio" | "document" | "any") => boolean;
export declare const uploadAvatar: (req: Request, res: Response) => Promise<void>;
export declare const uploadVideo: (req: Request, res: Response) => Promise<void>;
export declare const uploadDocument: (req: Request, res: Response) => Promise<void>;
export declare const uploadPropertyImages: (req: Request, res: Response) => Promise<void>;
export declare const serveSecureDocument: (req: Request, res: Response) => Promise<void>;
export declare const uploadMedia: (req: Request, res: Response) => Promise<void>;
//# sourceMappingURL=upload.controller.d.ts.map