...
const emailTemplate = (title, preheader, bodyHtml) => `
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
const btn = (text, href, color = '#4F46E5') => `<div style="text-align:center;margin:28px 0;">
...
const notifyBookingCreated = async (opts) => {
    await (0, exports.notify)({
        ...
        emailBodyHtml: emailTemplate('New Booking Request', `${opts.tenantName} wants to book your property`, `<h2 style="color:#1e293b;font-size:22px;margin:0 0 16px;">You have a new booking request!</h2>
...
const notifyPaymentReceipt = async (opts) => {
    await (0, exports.notify)({
        ...
        emailBodyHtml: emailTemplate('Payment Receipt', `We have securely received your payment of GHS ${opts.amount}`, `<h2 style="color:#1e293b;font-size:22px;margin:0 0 16px;">Authentic Payment Receipt</h2>
...
const notifyBookingStatusChanged = async (opts) => {
    ...
    await (0, exports.notify)({
        ...
        emailBodyHtml: emailTemplate(title, `Your booking status has been updated to ${opts.status}`, `<h2 style="color:#1e293b;font-size:22px;margin:0 0 16px;">${title}</h2>
       <p style="color:#475569;font-size:15px;line-height:1.7;">Hello <strong>${opts.tenantName}</strong>,</p>
...
const notifySubscriptionExpirySoon = async (opts) => {
    await (0, exports.notify)({
        ...
        emailBodyHtml: emailTemplate('Subscription Expiring Soon', `Your Akwaaba Homes subscription expires soon`, `<h2 style="color:#1e293b;font-size:22px;margin:0 0 16px;">Your subscription is expiring soon</h2>
       <p style="color:#475569;font-size:15px;line-height:1.7;">Hello <strong>${opts.landlordName}</strong>,</p>
...
const notifyPropertyApproval = async (opts) => {
    ...
    await (0, exports.notify)({
        ...
        emailBodyHtml: emailTemplate(`Property ${isApproved ? 'Approved' : 'Rejected'}`, `Your property listing decision is ready`, `<h2 style="color:#1e293b;font-size:22px;margin:0 0 16px;">Property ${isApproved ? 'Approved ✓' : 'Rejected ✗'}</h2>
       <p style="color:#475569;font-size:15px;line-height:1.7;">Hello <strong>${opts.landlordName}</strong>,</p>
...
const notifyPayoutSent = async (opts) => {
    await (0, exports.notify)({
        ...
        emailBodyHtml: emailTemplate('Payout Confirmed', `GHS ${opts.amount.toFixed(2)} transferred to your ${opts.bankOrNetwork} account`, `<h2 style="color:#1e293b;font-size:22px;margin:0 0 16px;">Withdrawal Complete 💸</h2>
       <p style="color:#475569;font-size:15px;line-height:1.7;">Hello <strong>${opts.landlordName}</strong>,</p>
...
const notifyLandlordVerification = async (opts) => {
    ...
    await (0, exports.notify)({
        ...
        emailBodyHtml: emailTemplate(`Landlord Verification ${opts.isVerified ? 'Approved' : 'Update Required'}`, opts.isVerified ? 'Your Verified Host badge is active' : 'Please check your submitted ownership documents', `<h2 style="color:#1e293b;font-size:22px;margin:0 0 16px;">Verification ${opts.isVerified ? 'Approved 🛡️' : 'Update Required'}</h2>
       <p style="color:#475569;font-size:15px;line-height:1.7;">Hello <strong>${opts.landlordName}</strong>,</p>
