...
async function sendSMS(to, message) {
    ...
    try {
        ...
        if (atUsername && atApiKey) {
            ...
            const response = await axios_1.default.post(atEndpoint, params.toString(), {
                headers: {
                    apiKey: atApiKey,
        ...
        if (process.env.HUBTEL_CLIENT_ID && hubtelSecret) {
            const authHeader = Buffer.from(`${process.env.HUBTEL_CLIENT_ID}:${hubtelSecret}`).toString('base64');
