...
exports.QRCodeEncoder = void 0;
exports.base32Encode = base32Encode;
exports.base32Decode = base32Decode;
exports.generateTOTPSecret = generateTOTPSecret;
exports.formatSecretKey = formatSecretKey;
exports.getTOTPUri = getTOTPUri;
...
exports.verifyAndConsumeRecoveryCode = verifyAndConsumeRecoveryCode;
...
function getTOTPUri(email, secret, issuer = 'Akwaaba Homes') {
    const encodedIssuer = encodeURIComponent(issuer);
    const encodedEmail = encodeURIComponent(email);
    return `otpauth://totp/${encodedIssuer}:${encodedEmail}?secret=${secret}&issuer=${encodedIssuer}&algorithm=SHA1&digits=6&period=30`;
}
function generateTOTPCode(secretBase32, timeStep = 30, t = Date.now()) {
    const counter = Math.floor(t / 1000 / timeStep);
    const counterBuf = Buffer.alloc(8);
    counterBuf.writeBigInt64BE(BigInt(counter));
    const key = base32Decode(secretBase32);
    const hmac = crypto_1.default.createHmac('sha1', key).update(counterBuf).digest();
...
function verifyTOTPCode(token, secretBase32, window = 1) {
    ...
    for (let w = -window; w <= window; w++) {
        ...
        if (crypto_1.default.timingSafeEqual(Buffer.from(cleanToken), Buffer.from(expected))) {
            return true;
        }
    }
    return false;
...
function generateRecoveryCodes(count = 8) {
    ...
    for (let i = 0; i < count; i++) {
        const buf = crypto_1.default.randomBytes(4).toString('hex').toUpperCase();
...
function verifyAndConsumeRecoveryCode(code, hashedCodesJson) {
    if (!code || !hashedCodesJson)
        return { valid: false, remainingHashedCodes: null };
    let hashedList = [];
    try {
        hashedList = JSON.parse(hashedCodesJson);
        if (!Array.isArray(hashedList))
            return { valid: false, remainingHashedCodes: null };
    }
    catch (e) {
        return { valid: false, remainingHashedCodes: null };
...
class QRCodeEncoder {
    ...
    setupPositionAdjustPattern() {
        ...
        for (let r = -2; r <= 2; r++) {
            for (let c = -2; c <= 2; c++) {
                if (this.modules[pos + r][pos + c] !== null)
                    continue;
...
exports.QRCodeEncoder = QRCodeEncoder;
function generateQRCodeSvg(data, size = 240) {
