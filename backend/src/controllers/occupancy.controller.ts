 ...
 */
export const getPropertyOccupancyMatrix = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const updateBedStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    try {
      getIO().emit('room_capacity_updated', { propertyId: property.id, bedId });
      getIO().emit('room_updated', { propertyId: property.id });
