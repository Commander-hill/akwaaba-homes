  ...
  try {
    ...
    if (status === 'VERIFIED') {
      ...
      if (shouldSuspend) {
        ...
        appCache.del(`user:me:${breach.tenantId}`);
      }
    }
    ...
    try {
      ...
      if (shouldSuspend) {
        emitToUser(breach.tenantId, 'session_revoked', { reason: 'Your account has been suspended due to verified contract breaches.' });
      }
      appCache.flushAll();
